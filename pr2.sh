...
function check_available_seats() {
    ...
    if grep -q "$movie_title - $showtime" "$BOOKINGS_FILE"; then
        booked_seats=$(grep "$movie_title - $showtime" "$BOOKINGS_FILE" | awk -F, '{print $5}' | tr '\n' ' ')
...
function book_ticket() {
    ...
    if grep -q "$movie_title - $showtime" "$BOOKINGS_FILE" && grep "$movie_title - $showtime" "$BOOKINGS_FILE" | grep -q ", $seat$"; then
